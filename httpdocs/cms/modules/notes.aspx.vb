...
Partial Class cms_modules_notes
...
#Region "Event Handlers"
            ...
            Select Case cmd.ToLower()
                Case "delete"
                    ...
                    For Each id As Integer In ids
                        completed = completed And NoteController.Delete(id, MyConn)
                    Next
    ...
    Protected Sub grid_DeleteRow(ByVal id As Integer) Handles egvGrid.RowDelete
        Try
            MyConn.Open()
            Dim completed As Boolean = NoteController.Delete(id, MyConn)
            If completed Then Master.Notifier.Success(Localization.GetResource("Resources.Local.delete.Success")) Else Master.Notifier.Warning(Localization.GetResource("Resources.Local.delete.Warning"))
            egvGrid.BindGrid(MyConn)
        Catch ex As Exception
            ExceptionHandler.ProcessException(ex, Master.Notifier, MyConn)
    ...
    Protected Sub lnkExport_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExport.Click
        Try
            MyConn.Open()
            Dim cq As New EntityStructure("modules/Note", AuthUser.Id, MyConn)
            Dim dt As DataTable = cq.GetCustomQuery(MyConn).Execute()
            Helper.DatatableToCSV(dt, "Notes-" & Guid.NewGuid().ToString())
        Catch ex As Exception
            ExceptionHandler.ProcessException(ex, Master.Notifier, MyConn)
        Finally
            MyConn.Close()
