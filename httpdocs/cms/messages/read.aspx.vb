...
Partial Class cms_messages_read
...
#Region "Event Handlers"
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            MyConn.Open()
            MessageController.DeleteMessage(MyConn, MessageId, AuthUser.Id, MessageUserTypes.User,
                                            IIf(MessageFilterType = MessageFilterTypes.Sent, MessageUserRoleTypes.Sent, MessageUserRoleTypes.Received)
                                            )
            Response.Redirect("list.aspx?type=" & MessageFilterType)
        Catch ex As Exception
            ExceptionHandler.ProcessException(ex, Master.Notifier, MyConn)
        Finally
            MyConn.Close()
    ...
    Protected Sub btnReply_Click(ByVal sender As Object, ByVal e As EventArgs)
        Response.Redirect("compose.aspx?reply=" & MessageId.ToString() & "&type=" & MessageFilterType)
    End Sub

    Protected Sub btnReplyAll_Click(ByVal sender As Object, ByVal e As EventArgs)
        Response.Redirect("compose.aspx?replyall=" & MessageId.ToString() & "&type=" & MessageFilterType)
    End Sub

    Protected Sub btnForward_Click(ByVal sender As Object, ByVal e As EventArgs)
        Response.Redirect("compose.aspx?forward=" & MessageId.ToString() & "&type=" & MessageFilterType)
    End Sub
...
#Region "Private Methods"
    ...
    Private Sub LoadMessage(ByVal conn As SqlConnection)
        ...
        MessageController.ReadMessage(conn, MessageId, AuthUser.Id, MessageUserTypes.User)
        ...
        End If
        Dim nxtPrev = MessageController.GetNextPrev(conn, AuthUser.Id, MessageUserTypes.User, IIf(MessageFilterType = MessageFilterTypes.Sent, MessageUserRoleTypes.Sent, MessageUserRoleTypes.Received), MessageId)
        If nxtPrev.NextId <> MessageId Then hypNext.NavigateUrl = "read.aspx?id=" & nxtPrev.NextId.ToString() & "&type=" & MessageFilterType Else hypNext.Visible = False
        If nxtPrev.PrevId <> MessageId Then hypPrevious.NavigateUrl = "read.aspx?id=" & nxtPrev.PrevId.ToString() & "&type=" & MessageFilterType Else hypPrevious.Visible = False
